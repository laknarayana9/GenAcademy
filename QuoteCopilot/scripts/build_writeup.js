...
function cell(text, { width, head = false, fill, bold = false, align } = {}) {
  return new TableCell({
    ...
    shading: { fill: head ? HEADERFILL : (fill || "FFFFFF"), type: ShadingType.CLEAR },
  ...
  return new Table({
    ...
    rows: rows.map((r, i) =>
      new TableRow({
        ...
        children: r.map((c) =>
          typeof c === "string"
            ? cell(c, { width: widths[r.indexOf(c)], head: i === 0 })
            : c),
      })),
  });
}
// build a table with header + zebra body
...
function rule() {
  return new Paragraph({ spacing: { after: 120 }, border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: ACCENT, space: 1 } }, children: [new TextRun({ text: "" })] });
}
...
const doc = new Document({
  ...
  styles: {
    ...
    paragraphStyles: [
      { id: "Title", name: "Title", basedOn: "Normal", next: "Normal",
        run: { size: 52, bold: true, color: NAVY, font: "Arial" },
  ...
  sections: [{
    ...
    headers: { default: new Header({ children: [new Paragraph({
      ...
      children: [new TextRun({ text: "QuoteCopilot", bold: true, color: NAVY, size: 18 }), new TextRun({ text: "\tThe Gen Academy · Week 3 — Build Your AI Agent", color: "777777", size: 18 })],
    })] }) },
    footers: { default: new Footer({ children: [new Paragraph({
      tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }],
      children: [new TextRun({ text: "Multi-Agent HO3 Underwriting Review", color: "999999", size: 16 }), new TextRun({ children: ["\tPage ", PageNumber.CURRENT], color: "999999", size: 16 })],
    })] }) },
    children: [
      new Paragraph({ style: "Title", children: [new TextRun("QuoteCopilot")] }),
      new Paragraph({ spacing: { after: 60 }, children: [new TextRun({ text: "A Multi-Agent Insurance Underwriting Review System for HO3 Homeowner Submissions", bold: true, color: ACCENT, size: 26 })] }),
      ...
      tocLine(4, "Datasets Used"),
      tocLine(5, "Prompts Used During AI-Assisted Coding"),
      tocLine(6, "Iterations We Tried"),
      tocLine(7, "Evaluation & Results"),
      tocLine(8, "Human-in-the-Loop & Failure Handling"),
      tocLine(9, "Limitations & Future Work"),
      tocLine(10, "Learnings & Observations"),
      ...
      h2("What it does autonomously vs. what it hands off"),
      dataTable([3120, 6240],
        ["Boundary", "Behavior"],
        [
      ...
      h2("How we know it worked"),
      p("Success is measured end-to-end (task completion), not single-shot accuracy: a demo user can complete the accept, missing-info pause/resume, wildfire follow-up, and refer/decline scenarios, each producing a cited packet, with the labeled evaluation suite passing its targets."),
      ...
      h2("4.1 Guideline corpus (retrieval source)"),
      ...
      // 5 Prompts
      h1("5. Prompts Used During AI-Assisted Coding"),
      p("The project was built with Claude Code (Opus) as the pair-programmer, working from an approved architecture spec. Two kinds of prompts mattered: (a) build prompts that drove the AI coding tool, and (b) the runtime agent prompts inside the system. Both are summarized below."),
      h2("5.1 Representative build prompts (to the AI coding tool)"),
      ...
      h2("5.2 Runtime agent prompt pattern"),
      p("Agents call a single guardrailed helper, complete_text(role, system, user, fallback), which returns the deterministic fallback whenever the LLM is unavailable or errors. Example — the Decision Packager phrases next steps but can never invent evidence:"),
      new Paragraph({ shading: { fill: "F4F4F4", type: ShadingType.CLEAR }, spacing: { before: 60, after: 120 }, border: { left: { style: BorderStyle.SINGLE, size: 12, color: "BBBBBB", space: 6 } }, children: [new TextRun({ font: "Courier New", size: 18, text:
        "system: “You are an underwriting assistant. Phrase clear next steps for the underwriter. Use only the supplied reason codes and citations. Do not invent facts or evidence.”" })] }),
      new Paragraph({ shading: { fill: "F4F4F4", type: ShadingType.CLEAR }, spacing: { after: 120 }, border: { left: { style: BorderStyle.SINGLE, size: 12, color: "BBBBBB", space: 6 } }, children: [new TextRun({ font: "Courier New", size: 18, text:
        "narrative = complete_text(“packager”, system, user, fallback=“\\n”.join(fallback_steps))" })] }),
      ...
      p("Product tests: 28 passing (unit: rules engine, rating tool, state schema; integration: full graph runs and API for accept, missing-info resume, decline→review, and review actions).", { }),

      // 8 HITL & failure handling
      h1("8. Human-in-the-Loop & Failure Handling"),
...
const outPath = path.join(outDir, "QuoteCopilot_Project_Writeup.docx");
